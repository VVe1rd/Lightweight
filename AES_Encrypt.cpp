...
int main_AES(unsigned char* random_message, int msg_length, unsigned char* random_key, int key_length, unsigned char*, int)
{
	int i;
...
// 	Part 2: ********************************************************
	...
	//if (FILE *fin = fopen("key.txt", "rb"))
	//{
	//	fread(Key, sizeof(Key[0]), Nk * 4, fin);
	//	fclose(fin);
	//}
	...
	for (int i = 0; i < Nb; i++) {
		in[i] = random_message[i];
	}

	//if (FILE *fin2 = fopen("random.txt", "rb"))
	//{	
	//	fread(in, sizeof(in[0]), Nb * 4, fin2);
	//	fclose(fin2);
	//}
